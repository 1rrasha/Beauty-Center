...
public class AppointmentTable extends Application {
	...
	public void start(Stage primaryStage) {
		...
		Button cancelAppointmentButton = new Button("Cancel");
		...
		cancelAppointmentButton.setStyle("-fx-font-weight: bold;");
		...
		HBox buttonsHbox = new HBox(insertAppointmentButton, searchAppointmentButton, exportButton, clearButton,
				cancelAppointmentButton, refreshButton);
		...
		insertAppointmentButton.setOnAction(event -> showInsertAppointmentDialog(userId));
	...
	private void showInsertAppointmentDialog(String userId) {
		Dialog<Appointment> dialog = new Dialog<>();
		dialog.setTitle("Reserve Appointment");
		dialog.setHeaderText("Please enter appointment details:");

		ButtonType insertButtonType = new ButtonType("Reseve", ButtonBar.ButtonData.OK_DONE);
		...
		TextField customerIDField = new TextField(userId);
		TextField serviceIDField = new TextField();

		customerIDField.setEditable(false); // Set customerIDField as non-editable
		...
		// Disable insert button if the service ID or time is empty
		serviceIDField.textProperty().addListener((observable, oldValue, newValue) -> {
	...
	private void cancelAppointment() {
		...
		// Check if the selected appointment belongs to the logged-in user
		if (selectedAppointment.getCustomer_id() != Integer.parseInt(userId)) {
			showAlert("Error", "You can only cancel your own appointments.");
			return;
		}
	...
	private void deleteAppointment(Appointment appointment) {
		try {
			...
			if (appointment.getCustomer_id() != Integer.parseInt(userId)) {
				showAlert("Error", "You can only cancel your own appointments.");
				return;
			}
			...
			PreparedStatement preparedStatement = connection.prepareStatement(query);
			preparedStatement.setInt(1, appointment.getAppointment_id());
			int rowsAffected = preparedStatement.executeUpdate();
